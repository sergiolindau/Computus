...
Math.deg2rad = Math.deg2rad || function (d) {
    return (Math.PI * d / 180);
};
const deg2rad = Math.deg2rad;
Math.rad2deg = Math.rad2deg || function (r) {
...
Math.isNumber = Math.isNumber || function isNumber(n) {
    return typeof n === 'number' && isFinite(n);
};
const isNumber = Math.isNumber;
Math.isPosInteger = Math.isPosInteger || function (n) {
    n = Number(n);
...
class Computus {
     ...
     */
    static doy2cal(y, d) {
        let leap = Computus.isleap(y);
        let i;
        for (i = 0; i < Computus.monthlength.length; i++) {
            if (d <= Computus.monthlength[i][leap + 1])
                break;
        }
     ...
     */
    static cal2jd(year, month, date, hours) {
     ...
     */
    static geomMeanLongSun_borkowski(T) {
     ...
     */
    static eccentricityEarthOrbit(T) {
     ...
     * @returns
     */
    static hourAngle(time, lng, eqtime) {
     ...
     */
    static hourAngleSunrise(lat, solarDec) {
     ...
     * @returns
     */
    static solarTimeUTC(JD, lng, time) {
        const eps = 1e-9;
        JD = round(JD);
     ...
     */
    static sunRiseNoonSetUTC(JD, lat, lng, risenoonset) {
        const eps = 1e-9;
        JD = round(JD);
     ...
     */
    static findSunRiseSet(JD, lat, lng, riseset, prevnext) {
    ...
    */
     ...
     * @returns
     */
    static polarLightNorthDarkSouth(lat, doy) {
     ...
     * @returns
     */
    static polarDarkNorthLightSouth(lat, doy) {
     ...
     * }
     */
    static sunRiseNoonSet(lat, lng, tz, jd, doy, daysave) {
        let result = {};
     ...
     */
    static refractionCorrection(exoatmElevation) {
        let result;
     ...
     * }
     */
    static sunPosition(lat, lng, tz, time, eqtime, dec) {
        let result = {};
        ...
        let csz = dsin(lat) * dsin(result.solarDec) +
    ...
    static sunpos(lat, lng, tz, time, eqtime, decl) {
        let result = {};
        result.eqtime = eqtime;
        result.decl = decl;
    ...
    static sunAnalemma(year, hour, lat, lng, tz) {
    ...
    static sunChart(year, lat, lng, tz) {
        let result = new Array(Computus.yeardays(year));
        let jdyear = Computus.cal2jd(year, 0, 1);
        for (let d = 0; d < result.length; d++) {
    ...
    static sunData(timepos) {
        ...
        result.sunpos = Computus.sunPosition(timepos.latitude, timepos.longitude, timepos.zone, (timepos.hours - timepos.daysave / 60) * 60 + timepos.minutes + timepos.seconds / 60, Computus.equationOfTime(T), Computus.sunDeclination(T));
        T = Computus.jd2jc(result.JD + 0.5 - timepos.zone / 24);
        result.sunposNoon = Computus.sunPosition(timepos.latitude, timepos.longitude, timepos.zone, result.timings.solNoonLST, Computus.equationOfTime(T), Computus.sunDeclination(T));
        T = Computus.jd2jc(result.JD - timepos.zone / 24);
        result.sunposMidnight = Computus.sunPosition(timepos.latitude, timepos.longitude, timepos.zone, 0, Computus.equationOfTime(T), Computus.sunDeclination(T));
        return result;
    }
    /**
     * ========== Moon's functions ==========
     ...
     */
    static truePhaseMoon1900(k, phase) {
        if (phase !== undefined) {
    ...
    static eccentricAnomaly(M, e, tol = 1e-14) {
...
Computus.moonphasename = {
    ...
    FR: ["Nouvelle lune", "Premier Quart", "Pleine Lune", "Dernier Quart"],
    DE: ["Neumond", "Erstes Viertel", "Vollmond", "Letztes Viertel"],
    LA: ["Neomenia", "Primum Quartum", "Plena luna", "Novissime Quartum"],
    HE: ["ירח חדש", "רבע ראשון", "ירח מלא", "רבע אחרון"],
