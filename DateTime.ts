...
/***********************************************************************
 ...
 =====================
   ...
   https://www.toptal.com/software/definitive-guide-to-datetime-manipulation
	...
	public static releaseYear = 2021;
	...
	protected TZString: string;
	protected LocalTZAdjust: number;
	protected DSTAdjust: number;
	protected TZDate: Date;
			...
			switch (arguments.length) {
				...
				case 1:
					if (Math.abs(Number(args[0]))<=19200) {
						if (this.isLocal) this.baseDate = new Date(Number(args[0]), 0, 1, 12, 0, 0, 0)
						else this.baseDate = new Date(Date.UTC(Number(args[0]), 0, 1, 12, 0, 0, 0)+this.LocalTZAdjust*6e4);
					}
					else {
					...
					break;
				case 2:
					if (this.isLocal) this.baseDate = new Date(Number(args[0]), Number(args[1]), 1, 12, 0, 0, 0)
					else this.baseDate = new Date(Date.UTC(Number(args[0]), Number(args[1]), 1, 12, 0, 0, 0)+this.LocalTZAdjust*6e4);
					break;
				case 3:
					if (this.isLocal) this.baseDate = new Date(Number(args[0]), Number(args[1]), Number(args[2]), 12, 0, 0, 0)
					else this.baseDate = new Date(Date.UTC(Number(args[0]), Number(args[1]), Number(args[2]), 12, 0, 0, 0)+this.LocalTZAdjust*6e4);
					break;
				case 4:
					if (this.isLocal) this.baseDate = new Date(Number(args[0]), Number(args[1]), Number(args[2]), Number(args[3]), 0, 0, 0)
					else this.baseDate = new Date(Date.UTC(Number(args[0]), Number(args[1]), Number(args[2]), Number(args[3]), 0, 0, 0)+this.LocalTZAdjust*6e4);
					break;
				case 5:
					if (this.isLocal) this.baseDate = new Date(Number(args[0]), Number(args[1]), Number(args[2]), Number(args[3]), Number(args[4]), 0, 0)
					else this.baseDate = new Date(Date.UTC(Number(args[0]), Number(args[1]), Number(args[2]), Number(args[3]), Number(args[4]), 0, 0)+this.LocalTZAdjust*6e4);
					break;
				case 6:
					if (this.isLocal) this.baseDate = new Date(Number(args[0]), Number(args[1]), Number(args[2]), Number(args[3]), Number(args[4]), Number(args[5]), 0)
					else this.baseDate = new Date(Date.UTC(Number(args[0]), Number(args[1]), Number(args[2]), Number(args[3]), Number(args[4]), Number(args[5]), 0)+this.LocalTZAdjust*6e4);
					break;
				case 7:
				case 8:
				default:
					if (this.isLocal) this.baseDate = new Date(Number(args[0]), Number(args[1]), Number(args[2]), Number(args[3]), Number(args[4]), Number(args[5]), Number(args[6]))
					else this.baseDate = new Date(Date.UTC(Number(args[0]), Number(args[1]), Number(args[2]), Number(args[3]), Number(args[4]), Number(args[5]), Number(args[6]))+this.LocalTZAdjust*6e4);
			}
	 ...
	 */
	public static UNIX(...args: Array<number>) {
		var temp;
		if (args.length>0) {
	...
	static parseTZOffset(tzoffs: number | string) {
/***********************************************************************
...
***********************************************************************/
		if (typeof(tzoffs)=='string') {
			...
			function func(){
				s = ((tzoffs as string).substring(0,1)=="-")?1:-1;
				if (s==-1) tzoffs = "+" + tzoffs;
				offs = s * (Number((tzoffs as string).substring(1,(tzoffs as string).length-2))*60 + Number((tzoffs as string).substring(3,(tzoffs as string).length)));
			}
...
***********************************************************************/
	...
	setLocalTZOffset(offset: number | string){
/***********************************************************************
...
***********************************************************************/
	...
	setDSTOffset(offs: number | string){
/***********************************************************************
...
***********************************************************************/
	...
	setUTC(year: number,month: number,date: number,hours: number,minutes: number,seconds: number,ms: number){
/***********************************************************************
...
***********************************************************************/
	...
	setLocal(year: number,month: number,date: number,hours: number,minutes: number,seconds: number,ms: number){
/***********************************************************************
...
/***********************************************************************/
	static timeToString(time: any,field?: number) {
...
/***********************************************************************/
	static timeToStringAMPM(time: any) {
...
/***********************************************************************/
	static dateToStringISO(date:any) {
...
/***********************************************************************/
	formatToString(format: string, larg: string){
/***********************************************************************
...
***********************************************************************/
	...
	if (format=="date") {
		...
		function fill0left(num: number | string,length: number){
	...
	else if (format=="json") return this.baseDate.toJSON();
	else if ((format=="utc")||(format=="gmt")) return this.baseDate.toUTCString();
...
***********************************************************************/
		for(var i=0; i<12; i++) {
			if (day<=DateTime.monthTable[i][leap?1:2]) break;
		}
		return i-1;
	}
...
***********************************************************************/
	...
	static julianDate(utc: DateTime | Date){
/***********************************************************************
...
***********************************************************************/
	...
	static modifiedJulianDate(utc: DateTime | Date) {
/***********************************************************************
...
***********************************************************************/
	...
	setModifiedJulianDate(j: number) {
/***********************************************************************
...
***********************************************************************/
	...
	static J2000(utc: DateTime | Date) {
/***********************************************************************
 Name: DateTime.J2000
 Type: Function
...
//1999 Dec 31, 0:00 UT
	static dayNumber(utc: DateTime | Date){
...
/**
 * From above has a console function log for debug purposes.
 */
 ...
 * @param dtz 
 */
console.logDateTime = function(dtz: DateTime) {
	console.log(dtz);
