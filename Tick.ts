...
function Tick(this: TTick, run_action?: Function, start_action?: Function, stop_action?: Function, tick_ms?: number) {
	let that = this;
	let tick_handle = 0;
	let is_running = false;
	let interval = (tick_ms)?tick_ms:200;
	this.runAction = function(tick: TTick) {
		console.log("Tick[handle:"+tick.getHandle()+"].runAction() at interval "+tick.getInterval())
	};
	this.startAction = function(tick: TTick) {
		console.log("Tick[handle:"+tick.getHandle()+"].startAction(), isRunning: "+tick.isRunning())
	};
	this.stopAction = function(tick: TTick) {
	...
	this.setInterval = function(i: number) {
		if (is_running) {
			clearInterval(tick_handle);
			interval = i;
			tick_handle = setInterval(that.runAction, interval, that);
